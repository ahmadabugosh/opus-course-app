...
- [ ] Configure custom domain (opus-course.learnopenclaw.ai) via Cloudflare DNS *(blocked: awaiting Cloudflare API token + zone access. `scripts/configure-cloudflare-domain.mjs` now supports either `CF_ZONE_ID` or `CF_ZONE_NAME`, with target fallback `CF_TARGET_CNAME` → `RAILWAY_PUBLIC_DOMAIN` → `railway domain --json`; also supports `--dry-run`, `CF_RECORD_NAME`, and `CF_PROXIED` for safer rollout once credentials are available)*
